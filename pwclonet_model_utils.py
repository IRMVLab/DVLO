...
def ProjectPCimg2SphericalRing(PC, img, Feature=None, H_input=64, W_input=1800):
    batch_size = PC.shape[0]
    ...
    for batch_idx in range(batch_size):
        ...
        PC_project_current = torch.zeros([H_input, W_input, 3]).cuda().detach()  # shape H W 3
        img_current = torch.zeros([H_input, W_input, 2]).cuda().detach()  # shape H W 2
        if Feature != None:
            Feature_project_current = torch.zeros([H_input, W_input, num_channel]).cuda().detach()
        ...
        cur_PC = cur_PC.to(torch.float32)
        cur_img = cur_img.to(torch.float32)

        ##print(img_current.is_cuda)
        ##print(cur_img.is_cuda)
        PC_project_current[iRow, iCol, :] = cur_PC[:, :]  # H W 3
        img_current[iRow, iCol, :] = cur_img[:, :]  # H W 2

        if Feature != None:
            Feature_project_current[iRow, iCol, :] = cur_Feature[:, :]

        PC_project_current = torch.reshape(PC_project_current, [1, H_input, W_input, 3])
        img_current = torch.reshape(img_current, [1, H_input, W_input, 2])
        if Feature != None:
            Feature_project_current = torch.reshape(Feature_project_current, [1, H_input, W_input, -1])
        ...
        if batch_idx == 0:
            ...
            PC_project_final = torch.cat([PC_project_final, PC_project_current], 0)  # b h w 3
            img_final = torch.cat([img_final, img_current], 0)

            if Feature != None:
                Feature_project_final = torch.cat([Feature_project_final, Feature_project_current], 0)

    if Feature != None:
        return PC_project_final, img_final, Feature_project_final
    else:
        return PC_project_final, img_final


def ProjectPCflag2SphericalRing(PC, flag, Feature=None, H_input=64, W_input=1800):
    batch_size = PC.shape[0]
    ...
    for batch_idx in range(batch_size):
        ...
        PC_project_current = torch.zeros([H_input, W_input, 3]).cuda().detach()  # shape H W 3
        flag_current = torch.zeros([H_input, W_input, 1]).cuda().detach()  # shape H W 1
        if Feature != None:
            Feature_project_current = torch.zeros([H_input, W_input, num_channel]).cuda().detach()
        ...
        cur_PC = cur_PC.to(torch.float32)
        cur_flag = cur_flag.to(torch.float32)

        ##print(img_current.is_cuda)
        ##print(cur_img.is_cuda)
        PC_project_current[iRow, iCol, :] = cur_PC[:, :]  # H W 3
        flag_current[iRow, iCol, :] = cur_flag[:, :]  # H W 1

        if Feature != None:
            Feature_project_current[iRow, iCol, :] = cur_Feature[:, :]

        PC_project_current = torch.reshape(PC_project_current, [1, H_input, W_input, 3])
        flag_current = torch.reshape(flag_current, [1, H_input, W_input, 1])
        if Feature != None:
            Feature_project_current = torch.reshape(Feature_project_current, [1, H_input, W_input, -1])

        if batch_idx == 0:
            PC_project_final = PC_project_current
            flag_final = flag_current
            if Feature != None:
                Feature_project_final = Feature_project_current

        else:
            PC_project_final = torch.cat([PC_project_final, PC_project_current], 0)  # b h w 3
            flag_final = torch.cat([flag_final, flag_current], 0)

            if Feature != None:
                Feature_project_final = torch.cat([Feature_project_final, Feature_project_current], 0)

    if Feature != None:
        return PC_project_final, flag_final, Feature_project_final
    else:
        return PC_project_final, flag_final


def ProjectPCflagxy2SphericalRing(PC, flag, xy, Feature=None, H_input=64, W_input=1800):
    batch_size = PC.shape[0]
    ...
    for batch_idx in range(batch_size):
        ...
        PC_project_current = torch.zeros([H_input, W_input, 3]).cuda().detach()  # shape H W 3
        flag_current = torch.zeros([H_input, W_input, 1]).cuda().detach()  # shape H W 1
        xy_current = torch.zeros([H_input, W_input, 2]).cuda().detach()  # shape H W 2
        if Feature != None:
            Feature_project_current = torch.zeros([H_input, W_input, num_channel]).cuda().detach()
        ...
        cur_PC = cur_PC.to(torch.float32)
        cur_flag = cur_flag.to(torch.float32)
        cur_xy = cur_xy.to(torch.float32)

        ##print(img_current.is_cuda)
        ##print(cur_img.is_cuda)
        PC_project_current[iRow, iCol, :] = cur_PC[:, :]  # H W 3
        flag_current[iRow, iCol, :] = cur_flag[:, :]  # H W 1
        xy_current[iRow, iCol, :] = cur_xy[:, :]  # H W 2
        ...
        PC_project_current = torch.reshape(PC_project_current, [1, H_input, W_input, 3])
        flag_current = torch.reshape(flag_current, [1, H_input, W_input, 1])
        xy_current = torch.reshape(xy_current, [1, H_input, W_input, 2])
        if Feature != None:
            Feature_project_current = torch.reshape(Feature_project_current, [1, H_input, W_input, -1])
        ...
        if batch_idx == 0:
            ...
            PC_project_final = torch.cat([PC_project_final, PC_project_current], 0)  # b h w 3
            flag_final = torch.cat([flag_final, flag_current], 0)
            xy_final = torch.cat([xy_final, xy_current], 0)

            if Feature != None:
                Feature_project_final = torch.cat([Feature_project_final, Feature_project_current], 0)

    if Feature != None:
        return PC_project_final, flag_final, xy_final, Feature_project_final
    else:
        return PC_project_final, flag_final, xy_final
